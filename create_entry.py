...
def select_from_list(options, prompt_text):
    """
    Displays a list of options and prompts the user to select one or enter a new one.
    """
    if options:
...
def create_new_entry():
    ...
    thinking = ""
    include_thinking = input("Include the model's 'Thinking' process? (y/n): ").strip().lower()
    if include_thinking == 'y':
        thinking = get_multiline_input("Thinking")

    # 3. Get Metadata
    print("\n--- Enter Metadata ---")
    difficulty = input("Difficulty (easy, medium, hard) [easy]: ").strip().lower() or "easy"
    ...
    custom_filename = input("Enter custom filename (or press Enter to auto-generate): ").strip()
